...
def merge_results(directory, filename_pattern, output_file, mse_limit=None):
    ...
    merged_dataframe = pd.concat(dataframes, axis=0).sort_values(by=["mean_squared_error"])
...
def get_best_means_dataframe(merged_dataframe, output_file=None, school_ids=[]):
    ...
    dataframes = []
    school_means_dataframes = [] # = pd.DataFrame(columns = ['school_id'] + merged_dataframe.columns)
    ...
    means_dataframe = pd.concat(dataframes, axis=0).groupby(list(columns_to_group)).mean().sort_values(by=['mean_squared_error']).reset_index()
    means_dataframe.insert(0,'school_id',0)

    if school_ids:
        school_means_dataframes.append(means_dataframe.drop(columns=[f"mse_{school}" for school in school_ids]).head(1))
        for school_id in school_ids:
            ...
            means_dataframe.loc[0, 'mean_squared_error'] = means_dataframe[f"mse_{school_id}"][0]
            school_means_dataframes.append(means_dataframe.drop(columns=[f"mse_{school}" for school in school_ids]).head(1))
    ...
    best_means_dataframe = pd.concat(school_means_dataframes, axis=0).sort_values(by=["school_id", "mean_squared_error"]).reset_index(drop=True)
    if output_file_exists:
...
def merge_repeats(*args, output_dir=os.getcwd(), schools=0):
    ...
    merged_dataframe = pd.concat(dataframes, axis=0).sort_values(dataframes[0].columns[1:-(1+schools)].to_list())

    # Write to `merged_mses.csv` (ordered by parameter values)
    merged_dataframe.to_csv(os.path.join(output_dir, "merged_mses.csv"), sep=",", encoding="utf-8", index=False)

    # Write to `lowest_to_highest_mses.csv` (ordered by MSE)
    merged_dataframe.sort_values(by=["mean_squared_error"], inplace=True)
    merged_dataframe.to_csv(os.path.join(output_dir, "lowest_to_highest_mses.csv"), sep=",", encoding="utf-8", index=False)
