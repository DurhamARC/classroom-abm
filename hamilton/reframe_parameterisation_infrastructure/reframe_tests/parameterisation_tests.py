...
class Parameterisation(rfm.RunOnlyRegressionTest):
    def __init__(self, n_processors, test_id):
        if n_processors == 8:
            self.time_limit = "1h45m" if "short" in os.environ["DATASET"] else "2h"
        elif n_processors == 16:
            self.time_limit = "1h30m" if "short" in os.environ["DATASET"] else "1h30"
        else:
            self.time_limit = "1h15m" if "short" in os.environ["DATASET"] else "1h"
